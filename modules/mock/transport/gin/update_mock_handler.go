...
// @Param Mock body model.MockUpdateRequest true "Update Mock"  // Correctly specify the request body
...
func UpdateMock(db *mongo.Database) func(*gin.Context) {
	return func(c *gin.Context) {
		...
		if err := business.UpdateMock(c.Request.Context(), id, &data); err != nil {
			...
			log.Error().Err(err).Str("request_id", c.GetString(middlewares.RequestIDKey)).Msg("failed to update Mock")
			return
		}
		...
		c.JSON(http.StatusOK, common.BaseApiResponse[model.Mock]{
			...
			Message:           "Mock is updated successfully",
			Data:              data,
		})
		log.Info().Str("Mock_id", id).Msg("Mock is updated successfully")
	}
}
