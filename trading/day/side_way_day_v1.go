...
type SidewayDayV1Signal struct {
	...
	SignalScore     SidewaySignalScore // Signal quality score
...
type SidewaySignalScore struct {
	TotalScore     float64
	MaxScore       float64
	Percentage     float64
...
type SidewayDayV1Strategy struct {
	rsiPeriod        int
	rsiOversold      float64
...
func NewSidewayDayV1Strategy() *SidewayDayV1Strategy {
	return &SidewayDayV1Strategy{
		rsiPeriod:        14,
		rsiOversold:      30,
		rsiOverbought:    70,
...
func (s *SidewayDayV1Strategy) AnalyzeWithSignalString(input tradingModels.CandleInput, symbol string) (*string, error) {
	if len(input.H1Candles) < 30 || len(input.M15Candles) < s.rsiPeriod {
		return nil, fmt.Errorf("insufficient data: need at least 30 H1 candles and %d M15 candles", s.rsiPeriod)
	}

	currentPrice := input.M15Candles[len(input.M15Candles)-1].Close

	// Detect range (support and resistance) from H1 candles
	...
	if currentPrice < support*0.998 || currentPrice > resistance*1.002 {
	...
	// SIDEWAY LOGIC: Mean reversion approach
	lenRSI := len(rsi)
	currentRSI := 50.0
	if lenRSI > 0 {
		currentRSI = rsi[lenRSI-1]
	}
	...
	// Pattern detection (supporting signals)
	...
	if supportDistance < 0.5 && rsiMeanReversionBuy && (priceBounceFromSupport || hasHammer || hasBullishEngulfing) {
		...
		signalScore := s.calculateSidewaySignalScore(input, side, currentPrice, support, resistance, rsi)
		...
		if !s.validateMinSLDistance(side, entry, support, resistance, input.M15Candles, input.H1Candles) {
		...
		signalStr := s.genSidewaySignalString(symbol, side, entry, input.M15Candles, input.H1Candles, signalScore, support, resistance, rangeWidth)
...
func (s *SidewayDayV1Strategy) AnalyzeWithSignalAndModel(input tradingModels.CandleInput, symbol string) (*string, *SidewayDayV1Signal, error) {
	if len(input.H1Candles) < 30 || len(input.M15Candles) < s.rsiPeriod {
		return nil, nil, fmt.Errorf("insufficient data: need at least 30 H1 candles and %d M15 candles", s.rsiPeriod)
	}
	...
	if supportDistance < 0.5 && rsiMeanReversionBuy && (priceBounceFromSupport || hasHammer || hasBullishEngulfing) {
		...
		signalStr := s.genSidewaySignalString(symbol, side, entry, input.M15Candles, input.H1Candles, signalScore, support, resistance, rangeWidth)
		signalModel := s.createSidewaySignalModel(symbol, side, entry, signalScore, support, resistance, rangeWidth, input.M15Candles, input.H1Candles)
...
// createSidewaySignalModel creates a SidewayDayV1Signal model from the analysis
func (s *SidewayDayV1Strategy) createSidewaySignalModel(symbol, side string, entry float64, signalScore SidewaySignalScore, support, resistance, rangeWidth float64, m15Candles []baseCandleModel.BaseCandle, h1Candles []baseCandleModel.BaseCandle) *SidewayDayV1Signal {
	// Calculate volatility profile
	volProfile := calculateDayTradingVolatilityProfile(m15Candles, h1Candles)

	// Calculate stop loss and take profit using ATR-based approach
	stopLoss, takeProfit, leverage := s.calculateATRBasedSLTP(side, entry, support, resistance, m15Candles, h1Candles)

	return &SidewayDayV1Signal{
		Symbol:          symbol,
		Decision:        side,
		Entry:           entry,
		StopLoss:        stopLoss,
...
// calculateATRBasedSLTP calculates stop loss and take profit based on ATR for sideway day trading
func (s *SidewayDayV1Strategy) calculateATRBasedSLTP(side string, entry, support, resistance float64, m15Candles []baseCandleModel.BaseCandle, h1Candles []baseCandleModel.BaseCandle) (stopLoss, takeProfit, leverage float64) {
	// Use H1 ATR for SL/TP calculation (more stable than M15 for day trading)
	var atrPercent float64
	if len(h1Candles) >= 15 {
		atrPercent = calcATRPercent(h1Candles, 14)
	} else {
	...
	if side == "BUY" {
		...
		actualSLBuffer := supportBuffer + (atrPercent * 0.7) // Slightly larger buffer for day trading
		...
		actualSLBuffer := resistanceBuffer + (atrPercent * 0.7)
		stopLoss = entry * (1 + actualSLBuffer)
	...
	baseLeverage := 0.025 / atrPercent // e.g., ATR 0.5% -> leverage 5x, ATR 1% -> leverage 2.5x
	...
	// Clamp leverage to reasonable bounds (1-10x for day trading sideway)
	leverage = math.Max(1.0, math.Min(10.0, leverage))
...
func (s *SidewayDayV1Strategy) validateMinSLDistance(side string, entry, support, resistance float64, m15Candles []baseCandleModel.BaseCandle, h1Candles []baseCandleModel.BaseCandle) bool {
	stopLoss, _, _ := s.calculateATRBasedSLTP(side, entry, support, resistance, m15Candles, h1Candles)
	...
	// Minimum SL distance requirements for day trading:
	// 1. At least 1.5x ATR (less strict than scalping)
	// 2. At least 0.2% absolute (to account for spread/slippage)
	minSLByATR := atrPercent * 1.5
	minSLAbsolute := 0.002 // 0.2%
...
// detectRange identifies support and resistance levels from H1 timeframe for day trading
func (s *SidewayDayV1Strategy) detectRange(input tradingModels.CandleInput) (support, resistance float64, rangeWidth float64) {
	// Use H1 for primary range detection (day trading)
	if len(input.H1Candles) < 24 {
		return 0, 0, 0
	}
	...
	// Look at last 48 H1 candles for range (2 days of data)
	lookback := 48
	if len(input.H1Candles) < lookback {
		lookback = len(input.H1Candles)
	}

	for i := len(input.H1Candles) - lookback; i < len(input.H1Candles); i++ {
		highs = append(highs, input.H1Candles[i].High)
		lows = append(lows, input.H1Candles[i].Low)
	}
	...
	// Validate range: should be reasonable for day trading (0.5% - 8%)
	if rangeWidth < 0.5 || rangeWidth > 8.0 {
		return 0, 0, 0
	}
...
// checkVolumeRequirement verifies volume is high enough for sideway day trading
func (s *SidewayDayV1Strategy) checkVolumeRequirement(candles []baseCandleModel.BaseCandle) bool {
	if len(candles) < 10 {
		return false
	}

	// Calculate average volume over last 10 M15 candles
	avgVolume := 0.0
...
func (s *SidewayDayV1Strategy) calculateSidewaySignalScore(input tradingModels.CandleInput, side string, currentPrice, support, resistance float64, rsi []float64) SidewaySignalScore {
	score := 0.0
	...
	rsiScore := s.scoreRSIMeanReversion(side, rsi)
	score += rsiScore
...
func (s *SidewayDayV1Strategy) getRecommendation(percentage float64) string {
	if percentage >= 85 {
...
func (s *SidewayDayV1Strategy) scoreRangePosition(side string, currentPrice, support, resistance float64) float64 {
	supportDistance := (currentPrice - support) / currentPrice * 100
	resistanceDistance := (resistance - currentPrice) / currentPrice * 100
...
func (s *SidewayDayV1Strategy) scoreVolumeConfirmation(candles []baseCandleModel.BaseCandle) float64 {
	if len(candles) < 10 {
		return 10.0
	}
...
func (s *SidewayDayV1Strategy) scoreRSIMeanReversion(side string, rsi []float64) float64 {
	if len(rsi) == 0 {
		return 10.0
	}
...
func (s *SidewayDayV1Strategy) scorePatternRecognition(candles []baseCandleModel.BaseCandle, side string) float64 {
	if len(candles) < 2 {
		return 7.5
	}
...
func (s *SidewayDayV1Strategy) scoreRangeQuality(support, resistance float64) float64 {
	if support == 0 || resistance == 0 {
		return 5.0
	}
	...
	// Ideal range width for day trading: 1% - 4%
	if rangeWidth >= 1.0 && rangeWidth <= 4.0 {
		return 15.0
	} else if rangeWidth >= 0.5 && rangeWidth <= 6.0 {
		return 12.0
	} else if rangeWidth >= 0.3 && rangeWidth <= 8.0 {
		return 8.0
	} else {
		return 5.0
	}
...
func (s *SidewayDayV1Strategy) detectBounceFromSupport(candles []baseCandleModel.BaseCandle, support float64) bool {
	if len(candles) < 3 {
		return false
	}
	...
	// Support distance threshold (1% for day trading)
	supportThreshold := support * 0.01
...
func (s *SidewayDayV1Strategy) detectBounceFromResistance(candles []baseCandleModel.BaseCandle, resistance float64) bool {
	if len(candles) < 3 {
		return false
	}
	...
	// Resistance distance threshold (1% for day trading)
	resistanceThreshold := resistance * 0.01
...
func (s *SidewayDayV1Strategy) detectBullishEngulfing(candles []baseCandleModel.BaseCandle) bool {
	if len(candles) < 2 {
		return false
	}
...
func (s *SidewayDayV1Strategy) detectBearishEngulfing(candles []baseCandleModel.BaseCandle) bool {
	if len(candles) < 2 {
		return false
	}
	prev := candles[len(candles)-2]
...
func (s *SidewayDayV1Strategy) detectHammer(candles []baseCandleModel.BaseCandle, maxBodyRatio float64) bool {
	if len(candles) < 1 {
		return false
	}
...
func (s *SidewayDayV1Strategy) detectShootingStar(candles []baseCandleModel.BaseCandle, maxBodyRatio float64) bool {
	if len(candles) < 1 {
		return false
	}
	c := candles[len(candles)-1]
...
func (s *SidewayDayV1Strategy) genSidewaySignalString(symbol, side string, entry float64, m15Candles []baseCandleModel.BaseCandle, h1Candles []baseCandleModel.BaseCandle, signalScore SidewaySignalScore, support, resistance, rangeWidth float64) string {
	var icon string
	if side == "BUY" {
		icon = "🟢"
	} else {
