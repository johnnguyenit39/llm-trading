...
func (s *TickVolumeScalpingStrategy) AnalyzeShortTermMarket(candles map[string][]baseCandleModel.BaseCandle) (*strategies.Signal, error) {
	...
	if latestVolume > latestVolumeMA*s.volumeThreshold {
		if latestPrice < latestVWAP && latestROC > 0 {
			...
			return &strategies.Signal{
				...
				Description: fmt.Sprintf("🚀 Tick/Volume Bar Scalping - BUY Signal %s\n\n"+
					"📊 Trade Setup:\n"+
					"• Entry Price: %.5f\n"+
					"• Stop Loss: %.5f (-%.2f%%)\n"+
					"• Take Profit: %.5f (+%.2f%%)\n"+
			...
			return &strategies.Signal{
				...
				Price: latestPrice,
				Time:  candles5m[len(candles5m)-1].OpenTime,
				Description: fmt.Sprintf("🔻 Tick/Volume Bar Scalping - SELL Signal %s\n\n"+
					"📊 Trade Setup:\n"+
					"• Entry Price: %.5f\n"+
					"• Stop Loss: %.5f (+%.2f%%)\n"+
					"• Take Profit: %.5f (-%.2f%%)\n"+
